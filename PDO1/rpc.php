...
//$voted=$query->rowCount($rating_conn->query("SELECT used_ips FROM $rating_dbname.$rating_tableName WHERE used_ips LIKE '%".$ip."%' AND id='".$id_sent."' "));
...
$newtotals = $rating_conn->query("SELECT `total_votes`, `total_value`, `used_ips` FROM $rating_tableName WHERE `id`='$id_sent' ") or die(" Error: " . $rating_conn->errorInfo());
...
$new_back[] .= '<li class="current-rating" style="width:' . @number_format($current_rating / $count,
        2) * $rating_unitwidth . 'px;">Current rating.</li>';
...
$new_back[] .= '</ul>';
$new_back[] .= '<p class="voted">' . $id_sent . '. Rating: <strong>' . @number_format($sum / $added,
        1) . '</strong>/' . $units . ' (' . $count . ' ' . $tense . ' cast) ';
